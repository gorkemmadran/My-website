    ...
    <section className={`home ${darkMode ? 'dark bg-black ' : ''}`}>
      <div className={`container mx-auto h-[86vh] dark:[86vh] xs:flex xs:justify-around xs:items-center xs:flex-col-reverse sm:flex-col-reverse md:flex-row lg:flex-row xl:flex-row 2xl:flex-row flex items-center justify-between `}>
        <div className='xs:flex xs:items-center xs:justify-center xs:flex-col'>
          ...
          <h1 className={`home-title font-opensans font-thin tracking-wide mt-2 text-3xl ${darkMode ? 'text-white' : 'text-black'}`}>I'M GÖRKEM MADRAN</h1>
          <h3 className={`text-xl font-opensans font-extralight mt-5 ${darkMode ? 'text-white' : 'text-black'}`}>Front End - React Developer</h3>
          <button className={`text-lg font-opensans hover:duration-700 hover:border hover:border-black hover:bg-white w-[170px] h-[48px] mt-[20px] ${darkMode ? 'bg-white text-black hover:text-black hover:bg-gray-300' : 'bg-black text-white hover:text-black hover:bg-white'}`}>Who am I ?</button>
        </div>
